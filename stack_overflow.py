...
async def extract_name(resume_text):
    try:
        ...
        for match_id, start, end in matches:
            extracted_name = doc[start:end].text.strip()
            # Check if the extracted name contains only alphabetic characters
            if extracted_name.isalpha():
                return extracted_name
            else:
                # If the extracted name contains symbols or numbers, append missing parts from the email
                email = re.search(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b", resume_text)
                if email:
                    email_parts = email.group().split('@')[0].split('.')
                    email_parts = [part for part in email_parts if part.isalpha()]  # Filter out non-alphabetic parts
                    missing_name_parts = [part for part in email_parts if part.lower() not in extracted_name.lower()]
                    extracted_name += ' ' + ' '.join(missing_name_parts)
                    return extracted_name
