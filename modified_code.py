...
def handle_resume():
    ...
    if file.filename == 'Poojacv.pdf':
        return jsonify({'file':file.filename})
       #return jsonify({'file': 'No selected file'})
    # print(file.filename)
    ...
    if file and file.filename.endswith('.pdf'):
        ...
        extracted_skills = loop.run_until_complete(extract_skills_from_resume(text, skills_list))
        extracted_education = loop.run_until_complete(extract_education_from_resume(text))
